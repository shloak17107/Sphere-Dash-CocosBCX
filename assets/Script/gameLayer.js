...
cc.Class({
    ...
    onLoad () {
            ...
            bcxAdapter.initSDK(function(res){
              ...
              if(res){
                bcxAdapter.login(function(res){
                    ...
                    self.accountDisplay.string = window.account_data; //== accountname
                    self.account_name = window.account_data;
                    bcxAdapter.getBalanceByAccount(window.account_data,function(res){
        ...
        // console.log("2");
        s(match && match.sharedRandomSeed,{global:true});
    ...
    onKeyDown: function (event) {
        switch(event.keyCode) {
            case cc.macro.KEY.a:
                if(this.ballImg){
                    this.ballImg.getComponent(cc.RigidBody).applyForceToCenter(cc.v2(-1*400000 , 0));
                }
                break;
            case cc.macro.KEY.d:
                if(this.ballImg){
                    this.ballImg.getComponent(cc.RigidBody).applyForceToCenter(cc.v2(400000 , 0));
                }
                break;
        }
    },
    ...
    //显示结算界面
    showGameOverLayer (){
        bcxAdapter.sendWinCocos(window.account_data,this.curScore,function(res){
                console.log("sendWinCocos",res)
            })
        celerx.submitScore(this.curScore);
